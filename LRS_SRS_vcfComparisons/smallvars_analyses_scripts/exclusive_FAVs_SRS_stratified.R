...
loadVCF <- function(path, label){
  ...
  system(str_glue('rm -rf srs.variants.vcf.gz 2>&1'), intern = TRUE)
  system(str_glue('cp {path} srs.variants.vcf.gz 2>&1'), intern = TRUE)
  vcf.o = readVcf("srs.variants.vcf.gz")
  ## annotate repeats
...
if(LOAD_CACHED_DATAFRAME){
  load('annotated.srsfav.gq20.df.rdata')
} else {
  srs.ann.df = lapply(probands, function(x){
    loadVCF(paste0('srs_only_FAVs_gq20/', x, '.fn.vcf.gz'), x)
  }) %>% bind_rows
  save(srs.ann.df, file='annotated.srsfav.gq20.df.rdata')
}

## Without GQ filtering
LOAD_CACHED_DATAFRAME = TRUE
if(LOAD_CACHED_DATAFRAME){
  load('annotated.srsfav.df.rdata')
} else {
  srs.ann.df = lapply(probands, function(x){
    loadVCF(paste0('srs_only_FAVs/', x, '.fn.vcf.gz'), x)
  }) %>% bind_rows
  save(srs.ann.df, file='annotated.srsfav.df.rdata')
}

## How many SRS-only FAVs are rare? How many of these overlap disease genes?
srs.ann.df <- srs.ann.df %>% group_by(seqnames,start,end) %>% mutate(AC=n())
srs.ann.rare.df <- srs.ann.df %>% filter(AF<0.001 & AC<=1)

x <- srs.ann.rare.df %>% filter(gene %in% dis.genes) %>% mutate(type=ifelse((str_length(ref)==1 & str_length(alt)==1), "snv", "indel"))
...
## How many in segdups and low mappability regions and homopolymers
print(paste(median((srs.ann.rare.df %>% filter(sd==TRUE) %>% group_by(label) %>% summarise(sd_favs=n()))$sd_favs), median((srs.ann.rare.df %>% filter(sd==TRUE) %>% group_by(label) %>% summarise(count=n()))$count)*100/median((srs.ann.rare.df %>% group_by(label) %>% summarise(count=n()))$count)))
print(paste(median((srs.ann.rare.df %>% filter(giab_lowmap==TRUE) %>% group_by(label) %>% summarise(count=n()))$count), median((srs.ann.rare.df %>% filter(giab_lowmap==TRUE) %>% group_by(label) %>% summarise(count=n()))$count)*100/median((srs.ann.rare.df %>% group_by(label) %>% summarise(count=n()))$count)))
print(paste(median((srs.ann.rare.df %>% filter(giab_lowcomp==TRUE) %>% group_by(label) %>% summarise(count=n()))$count), median((srs.ann.rare.df %>% filter(giab_lowcomp==TRUE) %>% group_by(label) %>% summarise(count=n()))$count)*100/median((srs.ann.rare.df %>% group_by(label) %>% summarise(count=n()))$count)))
print(paste(median((srs.ann.rare.df %>% filter(homopolymer==TRUE) %>% group_by(label) %>% summarise(count=n()))$count), median((srs.ann.rare.df %>% filter(homopolymer==TRUE) %>% group_by(label) %>% summarise(count=n()))$count)*100/median((srs.ann.rare.df %>% group_by(label) %>% summarise(count=n()))$count)))

## How many of these were INDEL contributions
srs.ann.rare.df <- srs.ann.rare.df %>% mutate(type=ifelse((str_length(ref)==1 & str_length(alt)==1), "snv", "indel"))
median((srs.ann.rare.df %>% filter(type=="indel") %>% group_by(label) %>% summarise(indels=n()))$indels)
median((srs.ann.rare.df %>% filter(type=="snv") %>% group_by(label) %>% summarise(snvs=n()))$snvs)
View(srs.ann.rare.df %>% filter(type=="snv"))

## How many SRS-only genes are disease genes?
## disease genes
...
srs_only_genes <- unique(srs.ann.rare.df$gene)
print(paste("No. of SRS-only genes in disease genes:", sum(srs_only_genes %in% dis.genes)))
print(paste("No. of SRS-only genes in recessive genes:", sum(srs_only_genes %in% recessive.genes)))

srs_only_genes[which(srs_only_genes %in% dis.genes)]
...
sro_favs_ALL <- srs.ann.df %>% group_by(label) %>% summarise(ALL=n())
sro_favs_RARE <- srs.ann.df %>% group_by(label) %>% summarise(RARE=count(AF<0.001 & AC<=1))
sro_favs_OMIM <- srs.ann.df %>% group_by(label) %>% filter(AF<0.001 & AC<=1) %>% summarise(OMIM=count(gene%in%dis.genes))

comb.df <- merge(merge(sro_favs_ALL, sro_favs_RARE, by="label", all=TRUE), sro_favs_OMIM, by="label", all=TRUE)
...
colors <- c("#D2848DFF", "#D4D2E5FF", "#A30248FF")
legend_labels <- c("ALL" = "Common in gnomAD or cohort",
                   "RARE" = "Rare",
                   "OMIM" = "Rare affecting disease genes")
a <- ggplot(comb.df_long, aes(x = label, y = value, fill = category)) +
  coord_flip() +
  ...
  ggtitle("Functionally Annotated Variants missed by LRS") + xlab("") + ylab("Number of SRS only FAVs")

ggsave(filename = "SuppFig3A.pdf", plot = a, width = 10, height = 6, units = "in", dpi=300)